...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget child = Ink(
      decoration: menuItemStyle.decoration,
...
class DropdownButton2State<T> extends State<DropdownButton2<T>> with WidgetsBindingObserver {
  ...
  TextStyle? get _textStyle => widget.style ?? context.theme.textTheme.titleMedium;

  Rect _getRect() {
    final TextDirection? textDirection = Directionality.maybeOf(context);
    const EdgeInsetsGeometry menuMargin = EdgeInsets.zero;
    final NavigatorState navigator = Navigator.of(context, rootNavigator: dropdownStyle.useRootNavigator);
  ...
  double get _denseButtonHeight {
    final double fontSize = _textStyle!.fontSize ?? context.theme.textTheme.titleMedium!.fontSize!;
    final double textScaleFactor = MediaQuery.of(context).textScaler.scale(fontSize);
  ...
  Widget build(BuildContext context) {
    ...
    Widget result = DefaultTextStyle(
      style: _enabled ? _textStyle! : _textStyle!.copyWith(color: context.theme.disabledColor),

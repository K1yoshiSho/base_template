...
class _SlidingUpPanelState extends State<SlidingUpPanel> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _ac = AnimationController(
        ...
        value: widget.defaultPanelState == PanelState.snapped
            ? (widget.snapPoint ?? 0.0)
            : (widget.defaultPanelState == PanelState.closed ? 0.0 : 1.0) //set the default panel state (i.e. set initial value of _ac)
        )
      ..addListener(() {
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    return Listener(
      onPointerDown: (PointerDownEvent e) {
        ...
        } else if (result.path.any((entry) => entry.target.runtimeType == IgnoreDraggableWidgetWidgetRenderBox)) {

...
class InlineParser {
  InlineParser(this.source, this.document) : _stack = <TagState>[] {
    ...
    final documentHasCustomInlineSyntaxes = document.inlineSyntaxes.any((s) => !document.extensionSet.inlineSyntaxes.contains(s));
    ...
    syntaxes
      ...
      ..insertAll(1, [LinkSyntax(linkResolver: document.linkResolver), ImageSyntax(linkResolver: document.imageLinkResolver)]);
  }
...
class _DelimiterRun {
  _DelimiterRun._({this.char, this.length, this.isLeftFlanking, this.isRightFlanking, this.isPrecededByPunctuation, this.isFollowedByPunctuation});
  ...
  bool tryMatch(InlineParser parser) {
    ...
    if (closingDelimiterRun != null && closingDelimiterRun.canClose) {
      ...
      final oneRunOpensAndCloses =
          (openingDelimiterRun!.canOpen && openingDelimiterRun!.canClose) || (closingDelimiterRun.canOpen && closingDelimiterRun.canClose);

...
class Rule {
  ...
  Rule(this.name, {this.filters, this.filterFn, this.replacement, this.append}) : _realFilterFn = _buildFilterFn(filters, filterFn);
  ...
  static Rule findRule(Node node) {
    if (_customRules.isNotEmpty) {
      var customRule = _customRules.firstWhereOrNull((rule) => rule._check(node));
      if (customRule != null) return customRule;
    }

    return _commonMarkRules.firstWhere((rule) => rule._check(node), orElse: () => _BaseRules.defaultRule);
  }
...
abstract class _BaseRules {
  static final Rule defaultRule = Rule('default', filters: ['default'], replacement: (content, node) {
...
abstract class _CommonRules {
  static final Rule paragraph = Rule('paragraph', filters: ['p'], replacement: (content, node) {
    return '\n\n$content\n\n';
  });

  static final Rule lineBreak = Rule('lineBreak', filters: ['br'], replacement: (content, node) {
    return '${getStyleOption('br')}\n';
  });

  static final Rule heading = Rule('heading', filters: ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'], replacement: (content, node) {
  ...
  static final Rule blockquote = Rule('blockquote', filters: ['blockquote'], replacement: (content, node) {
    var convertContent = content.replaceAll(RegExp(r'^\n+|\n+$'), '').replaceAll(RegExp(r'^', multiLine: true), '> ');
    return '\n\n$convertContent\n\n';
  });

  static final Rule list = Rule('list', filters: ['ul', 'ol'], replacement: (content, node) {
  ...
  static final Rule listItem = Rule('listItem', filters: ['li'], replacement: (content, node) {
    var convertContent =
    ...
    var postfix = ((node.nextSibling != null) && !RegExp(r'\n$').hasMatch(convertContent)) ? '\n' : '';
    return '$prefix$convertContent$postfix';
  });

  static final Rule indentedCodeBlock = Rule('indentedCodeBlock', filterFn: (node) {
    return getStyleOption('codeBlockStyle') == 'indented' && node.nodeName == 'code' && node.parentElName == 'pre';
  ...
  static final Rule fencedCodeBlock = Rule('fencedCodeBlock', filterFn: (node) {
    return getStyleOption('codeBlockStyle') == 'fenced' && node.nodeName == 'code' && node.parentElName == 'pre';
  }, replacement: (content, node) {
  ...
  static final Rule horizontalRule = Rule('horizontalRule', filters: ['hr'], replacement: (content, node) {
  ...
  static final Rule emphasis = Rule('emphasis', filters: ['em', 'i'], replacement: (content, node) {
    if (content.trim().isEmpty) return '';
    return getStyleOption('emDelimiter') + content + getStyleOption('emDelimiter');
  });

  static final Rule strong = Rule('strong', filters: ['strong', 'b'], replacement: (content, node) {
    if (content.trim().isEmpty) return '';
    return getStyleOption('strongDelimiter') + content + getStyleOption('strongDelimiter');
  });

  static final Rule strike = Rule('strike', filters: ['strike', 's', 'del'], replacement: (content, node) {
    if (content.trim().isEmpty) return '';
    return '~~$content~~';
  });
  ...
  static final Rule code = Rule('code', filterFn: (node) {
    ...
    var leadingSpace = '';
    var trailingSpace = '';
    var matches = RegExp(r'`+').allMatches(content).map((match) => match.group(0)).toList();
  ...
  static final Rule image = Rule('image', filters: ['img'], replacement: (content, node) {
...
abstract class _TableRules {
  static final Rule tableCell =
      Rule('tableCell', filters: ['th', 'td'], replacement: (content, node) => cell(content, node) + spannedCells(node, ''));

  static final Rule tableRow = Rule('tableRow', filters: ['tr'], replacement: (content, node) {
    var borderCells = '';
  ...
  static final Rule table = Rule('table', filters: ['table'], replacement: (content, node) {
  ...
  static final Rule tableSection = Rule('tableSection', filters: ['thead', 'tbody', 'tfoot'], replacement: (content, node) => content);

  static final Rule captionSection = Rule('captionSection', filters: ['caption'], replacement: (content, node) {

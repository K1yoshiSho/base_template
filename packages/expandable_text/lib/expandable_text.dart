...
class ExpandableTextState extends State<ExpandableText> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final text =
        _textSegments.isNotEmpty ? TextSpan(children: _buildTextSpans(_textSegments, effectiveTextStyle, null)) : TextSpan(text: widget.text);
          ...
          final text = _textSegments.isNotEmpty
              ? TextSpan(
                  children: _buildTextSpans(
                      _expanded ? _textSegments : parseText(widget.text.substring(0, max(endOffset, 0))), effectiveTextStyle!, recognizer),
                )
              : TextSpan(
